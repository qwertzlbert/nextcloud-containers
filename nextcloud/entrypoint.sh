...
if expr "$1" : "apache" 1>/dev/null || [ "$1" = "php-fpm" ] || [ "${NEXTCLOUD_UPDATE:-0}" -eq 1 ]; then
    ...
    rsync -rlD /php-fpm/config/ ${PHP_INI_DIR}-fpm.d/

    # check if minimum required config files were provided
    if [ ! -f ${PHP_INI_DIR}/conf.d/opcache.ini ] || [ ! -f ${PHP_INI_DIR}/conf.d/docker-php-ext-apcu.ini ]; then
        echo "Can't start Nextcloud minimum required config missing"
        exit 1
    fi    
